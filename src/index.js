...
const updateTsconfig = () => {
  return new Promise(resolve => {
    ...
    replace(options, err => {
      ...
      console.log(
        `Modified files: ${options.files} from: "baseUrl": "./", to: ${options.to}`
      );
      resolve();
    });
  });
...
const updateAngularJson = () => {
  return new Promise((resolve, reject) => {
    ...
    replace(options, err => {
      ...
      console.log(
        `Modified files: ${options.files} from: "src/styles.scss", to: "${options.to}"`
      );
      resolve();
    });
  });
...
const updateTslint = () => {
  return new Promise((resolve, reject) => {
    ...
    replace(options, err => {
      ...
      console.log(
        `Modified files: ${options.files} from: "app", to: "${options.to}"`
      );
      resolve();
    });
  });
    ...
    fs.readFile(options.files, function(err, data) {
      ...
      if (!data.includes("us-splash-screen")) {
        replace(options, err => {
          ...
          console.log(
            `Modified files: ${options.files} from: "", to: "${options.to}"`
          );
          resolve();
        });
      } else {
...
const npmSplashScreen = () => {
  return new Promise(resolve => {
    npm.load(function(err) {
      ...
      npm.commands.install(["@ultrastark/us-splash-screen"], function(
        err,
        data
      ) {
        if (err) {
        ...
        console.log(
          "@ultrastark/us-splash-screen and @angular/animations installed"
        );
        resolve();
      });
